...
window.instaWindow = (baseDom) => {
  ...
  const copyrightDom = () => {
    ...
    if (hostname != 'localhost' && hostname != 'insta-window-tool.web.app') {
      const gaImg = document.createElement('img');
      gaImg.className = 'iswd-tracking-img';
      const TID = 'UA-142501014-2';
      gaImg.src = `//www.google-analytics.com/collect?v=1&tid=${TID}&cid=1&t=event&ec=views&ea=${hostname}&el=${location.href}`;
      wrapper.appendChild(gaImg);
      // Proプラン トラッキング
      const proGaImg = gaImg.cloneNode();
      proGaImg.src = `//www.google-analytics.com/collect?v=1&tid=${TID}&cid=1&t=event&ec=proViews&ea=${hostname}&el=${location.href}`;
      wrapper.appendChild(proGaImg);
    }
    return wrapper;
  }
